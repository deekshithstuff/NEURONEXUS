$base = 'c:\Users\Deekshith\Downloads\pytest_cache\NEURONEXUS_TEST_DATASET_90\NEURONEXUS_TEST_DATASET_90'
...
base = Path(r'c:\Users\Deekshith\Downloads\pytest_cache\NEURONEXUS_TEST_DATASET_90\NEURONEXUS_TEST_DATASET_90')
api = 'http://127.0.0.1:8001'
summary = {cat: {'ok': 0, 'fail': 0, 'files': []} for cat in ['01_valid_complete','02_citation_issues','03_structure_issues','04_methodology_quality','05_journal_scope','06_writing_quality','07_formatting','08_tables_figures_equations','09_edge_cases']}
    ...
    for f in sorted(cat.glob('*.docx')):
        try:
            with open(f, 'rb') as fh:
                up = requests.post(f'{api}/api/documents/upload', files={'file': (f.name, fh, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')}, timeout=60)
            ...
            report = requests.post(f'{api}/api/report/generate', json={'document_id': doc_id, 'analysis': data, 'quality_analysis': quality.json() if quality.status_code == 200 else {}, 'selected_journal': 'nature'}, timeout=60)
            ...
            if ok:
                ...
                summary[cat.name]['fail'] += 1
                summary[cat.name]['files'].append({'file': f.name, 'quality_status': quality.status_code, 'match_status': match.status_code, 'report_status': report.status_code})
